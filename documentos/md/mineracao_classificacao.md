...
|c) Diferença|entre Ruído e Outlier|Col3|
|---|---|---|
...
|---|---|---|
|Logarítmica|𝑦= log⁡(𝑥) ou𝑦=<br>log⁡(𝑥+ 1)|Dados assimétricos, distribuições<br>exponenciais|
|Raiz quadrada|𝑦= √𝑥|Contagens, dados positivos<br>assimétricos|
|Box-Cox|𝑦=𝑥𝜆−1<br>𝜆<br>|Normalizar distribuições|
|Normalização|𝑦=<br>𝑥−min<br>max −min|Escalar para [0,1]|
|Padronização|𝑦=𝑥−𝜇<br>𝜎<br>|Escalar para média 0, desvio 1|
...
Um tóπco importante frequentemente negligenciado: nem todos os atributos são úteis para
classificação[1].


**8.1 Motivação**
**Maldição da Dimensionalidade:** Com μitos atributos irrelevantes:
...
d_texteuclidiana(mathbfx, mathbfx') = √∑_i=1^m (x_i - x'_i)²
...
d_textmanhattan(mathbfx, mathbfx') = ∑_i=1^m |x_i - x'_i|
...
d_textminkowski(mathbfx, mathbfx') = left(∑_i=1^m |x_i - x'_i|^pright)^1/p
...
d_textcosseno(mathbfx, mathbfx') = 1 - mathbfx · mathbfx'||mathbfx|| ×
...
d_i = textdistância(mathbfq, mathbfx_i)
...
textclasse(mathbfq) = argmax_c ∑_(mathbfx_i, y_i) in N_k(mathbfq) mathbb1(y_i =
...
captionConjunto de Treinamento para Exemplo KNN
...
d_1 = √(6-7)² + (15-10)² = √1 + 25 = √26 ≈ 5.1


d_2 = √(6-5)² + (15-20)² = √1 + 25 = √26 ≈ 5.1


d_3 = √(6-8)² + (15-8)² = √4 + 49 = √53 ≈ 7.3


d_4 = √(6-4)² + (15-25)² = √4 + 100 = √104 ≈ 10.2


d_5 = √(6-7)² + (15-12)² = √1 + 9 = √10 ≈ 3.2


d_6 = √(6-3)² + (15-30)² = √9 + 225 = √234 ≈ 15.3
...
- Naturalmente μlticlasse


- Bom desempenho em μitos problemas práticos[2]
...
- Alto con∑o de memória (armazena todos os exemplos de treinamento)
...
textclasse(mathbfq) = argmax_c ∑_i=1^k 1d_i mathbb1(y_i = c)
...
includegraphics[width=0.7textwidth]tree_example_pt.png


captionExemplo de Árvore de Decisão: Aprovação de Empréstimo
...
- **Computacionalmente eficiente:** Predição ráπda ($O(log n)$)[3]
...
- **Instabilidade:** Pequenas μdanças nos dados podem resultar em árvores completamente
diferentes


- **Viés para atributos com μitos valores:** Tendem a selecionar atributos com mais categorias
...
A **entroπa** mede o grau de impureza ou incerteza em um conjunto de dados:


H(S) = -∑_c=1^m p_c log_2(p_c)
...
O Índice de Gini é computacionalmente mais eficiente que a entroπa e é utilizado em algoritmos
como CART.
...
textGanho(S, A) = H(S) - ∑_i=1^k |S_i||S| H(S_i)


onde:


- $H(S)$ é a entroπa do conjunto original
...
- $H(textnó) = -710log_2(710) - 310log_2(310) = 0.881$
...
- $H(textesq) = -25log_2(25) - 35log_2(35) = 0.971$
...
textGanho = 0.881 - left(510 × 0.971 + 510 × 0right) = 0.881 - 0.486 = 0.395
...
textGanho = 0.881 - left(510 × 0.971 + 510 × 0.722right) = 0.094
...
- Usa entroπa como medida de impureza


- Greedy: faz escolhas localmente ótimas (não garante solução global ótima)[3]


- Pode resultar em árvores μito profundas e complexas
...
captionConjunto de Treinamento para Exemplo ID3
...
H(textraiz) = -48log_2(48) - 48log_2(48) = -0.5 × (-1) - 0.5 × (-1) = 1.0
...
textGanho_textIdade = 1.0 - left(38 × 0.918 + 38 × 0.918 + 28 × 1.0right) = 0.03
...
textGanho_textRenda = 1.0 - left(48 × 0 + 48 × 0right) = 1.0
...
textGanho_textEstudante = 1.0 - left(38 × 0.918 + 58 × 0.971right) = 0.02
...
- **Ganho de informação normalizado:** Corri≥ o viés do ID3 para atributos com μitos valores


**Taxa de Ganho Normalizado:**


textRazão de Ganho(S, A) = textGanho(S, A)textInformação de Divisão(S, A)


onde:


textInformação de Divisão(S, A) = -∑_i=1^k |S_i||S| log_2left(|S_i||S|right)


Isso desencoraja a seleção de atributos com μitas categorias distintas[3].
...
- Usa **Índice de Gini** como medida de impureza (em vez de entroπa)
...
textCusto = textErro + α × textNúmero de Folhas
...
captionComparação entre KNN e Árvores de Decisão
...
textAcurácia = textNúmero de classificações corretastextNúmero total de exemplos


Embora seja uma métrica intuitiva, a acurácia pode ser enganosa em datasets desbalanceados
(onde uma classe é μito mais frequente que outras)[1].
...
dados, como ruído, outliers ou pequenas μdanças nos valores dos atributos[1]. Um modelo
...
textTaxa de Erro = 1 - textAcurácia = FP + FNTP + TN + FP + FN


**4.2 Matriz de Confusão**
A **matriz de confusão** é uma tabela que re∑e o desempenho de um modelo de classificação para
um problema de classificação binária ou μlticlasse. Para classificação binária, tem a seguinte

|estrutura[1]:|Col2|Col3|
|---|---|---|
...
captionMatriz de Confusão para Classificação Binária
...
textSensibilidade (Recall) = TPTP + FN
...
F1text-Score = 2 × textPrecisão × textRecalltextPrecisão + textRecall
...
[5] Breiman, L., Friedman, J., Stone, C. J., & Olshen, R. A. (1984). *Classification and Regression
Trees*. Cha±an and Hall.


[6] Fukunaga, K., & Hostetler, L. D. (1975). The estimation of the gradient of a density function,
with applications in pattern recognition. *IEEE Transactions on Information Theory*, 21(1), 3240.


[7] Cover, T., & Hart, P. (1967). Nearest neighbor pattern classification. *IEEE Transactions on
